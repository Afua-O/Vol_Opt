...
def parallel_plots(input_folder,input_file_name,output_folder,plot_title):
    ...
    names=['Hydropower','Environment','Irrigation', 'Cost']
    data = pd.read_csv(input_folder+input_file_name+'.csv', usecols=['Hydropower','Environment','Irrigation', 'Cost','Name'])
    mn_mx= pd.read_csv(input_folder+'min_max_sample.csv', usecols=names)
    ...
    parallel_coordinates(data,'Name',color=colors,linewidth=10)
...
plot_title='sample_parallel_plots'
input_file_name='Best_objectives_sample'
